...
- `dist/` is generated by packaging scripts; source of truth remains repository root files.
...
- `npm run copy`: create `dist/` and copy runtime/docs/templates into it.
...
- Keep global mutable state limited and explicit, currently `CLIPBOARD_IMAGE_LOCKED` and `CLIPBOARD_HIDDEN_MODE`.
...
- Avoid splitting files unless the user asks for it; keep the module lean, but refactor aggressively inside the file when repeated logic appears.
